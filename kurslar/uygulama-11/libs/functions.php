<?php
    function kursEkle(&$kurslar,string $baslik,string $altBaslik,string $resim,string $yayinTarihi,int $yorumSayisi=0,int $begeniSayisi=0,bool $onay=true){
        ...
        $kurslar=array_merge($kurslar,$yeni_kurs);//iki tane dizi elemanını birleştirir
    }
    ...
    function safe_html($data){
        $data = trim($data);//boşlukları göz ardı eder
        $data=stripslashes($data);//süzme işlemi yapar
        $data=htmlspecialchars($data);//etiketlerin html karşılığını getirir
        return $data;
    }
?>